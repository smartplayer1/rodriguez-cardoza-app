...
export default function NotasCredito() {
  ...
  const sortedNotas = [...filteredNotas].sort((a, b) => {
    const compareA: string | number | undefined = a[sortColumn];
    const compareB: string | number | undefined = b[sortColumn];
  ...
  const handleToggleItem = (itemId: string) => {
    ...
    if (newSelected.has(itemId)) {
      ...
      const item = selectedFactura?.items.find((i: FacturaItem) => i.id === itemId);
      if (item) {
        setItemQuantities({
          ...itemQuantities,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const items: NotaCreditoItem[] = Array.from(selectedItems).map(itemId => {
      const facturaItem = selectedFactura?.items.find((i: FacturaItem) => i.id === itemId);
      return {
  ...
  const handleInputChange = (field: keyof NotaCredito, value: unknown) => {
                ...
                <tbody className="divide-y divide-border">
                  {viewingNota.items.map(item => (
                    <tr key={item.id}>
                      ...
                      <td className="px-6 py-3 text-sm text-foreground text-right font-mono">C${item.precioUnitario.toFixed(2)}</td>
                      <td className="px-6 py-3 text-sm text-foreground text-right font-mono">C${item.subtotal.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleInputChange('estado', e.target.value as 'borrador' | 'emitida' | 'anulada')}
              ...
              <div className="bg-surface rounded elevation-2 p-6 mb-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-border">
                      {selectedFactura.items.map((item: FacturaItem) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative">
              ...
              <select
                value={filterEstado}
                onChange={(e) => {
                  setFilterEstado(e.target.value as 'todos' | 'borrador' | 'emitida' | 'anulada');
                  setCurrentPage(1);
                }}
                className="w-full pl-10 pr-4 py-2 bg-input-background border-b-2 border-border 
