...
export default function GestionesHeaderTable({ records }: Props) {
  const [selectedRecord, setSelectedRecord] = useState<CashManagementRecord | null>(null);
          ...
          <table className="min-w-full divide-y divide-border/60 text-sm">
            <thead className="bg-muted/40">
              <tr>
                ...
                <th className="px-4 py-3 text-center font-medium text-muted-foreground">Detalle</th>
              </tr>
            </thead>
                  ...
                  <tr key={record.id} className="align-top transition-colors hover:bg-muted/15">
                    ...
                    <td className="px-4 py-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center rounded-xl border border-border px-3 py-1.5 text-xs text-foreground transition-colors hover:bg-accent"
                      >
                        Ver detalle
                      </button>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-4xl max-h-[90vh] overflow-auto rounded-3xl border border-border/60 bg-surface p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-foreground">{selectedRecord.cashRegisterName}</h3>
                <p className="text-sm text-muted-foreground">
                  Responsable: {selectedRecord.responsibleEmployeeName} · Tasa {selectedRecord.exchangeRateNioPerUsd} NIO/USD
                </p>
              </div>
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              <DenominationTable title="Denominaciones apertura" items={selectedRecord.openingDenominations} />
              <DenominationTable title="Denominaciones cierre" items={selectedRecord.closingDenominations} />
            </div>
          </div>
        </div>
      ) : null}
