...
export default function CerrarCajaModal({
  ...
  const estimatedTotalNio = useMemo(() => {
    return rows.reduce((sum, row) => {
      ...
      if (row.currency === "USD") {
        return sum + lineTotal * record.exchangeRateNioPerUsd;
      }

      return sum + lineTotal;
    }, 0);
  }, [rows, record.exchangeRateNioPerUsd]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await closeCashManagement(record.id, payload);
      router.refresh();
      onClose();
    } catch (error) {
      setErrorMessage(
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid gap-3 rounded-2xl border border-border/60 bg-background/60 p-4 text-sm text-muted-foreground sm:grid-cols-2">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-foreground">
                  {record.openedByUserName}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>Balance actual NIO</span>
                <span className="text-foreground">
                  {formatCurrency(record.balance.nio, "NIO")}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>Balance actual USD</span>
                <span className="text-foreground">
                  {formatCurrency(record.balance.usd, "USD")}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>Esperado NIO</span>
                <span className="text-foreground">
                  {formatCurrency(record.expectedNioAtClose, "NIO")}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span>Esperado USD</span>
                <span className="text-foreground">
                  {formatCurrency(record.expectedUsdAtClose, "USD")}
                </span>
              </div>
            </div>
