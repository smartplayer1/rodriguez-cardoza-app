...
export default function EditarFactura({
  ...
  const [cashManagementTouched, setCashManagementTouched] = useState(false);
  ...
  useEffect(() => {
    ...
    setCashManagementTouched(false);
  }, [invoice, isOpen]);

  useEffect(() => {
    if (!isOpen) return;
    void loadOpenCashManagementRecords();
  }, [isOpen, loadOpenCashManagementRecords]);

  const computedTotals = useMemo(() => {
    const grossTotal = detailsForm.reduce(
    ...
    const lineDiscountTotal = detailsForm.reduce(
      (acc, detail) => acc + detail.lineDiscount,
      0,
    );
    const generalDiscountTotal = detailsForm.reduce(
      (acc, detail) => acc + detail.generalDiscount,
      0,
    );
    const netTotal =
      grossTotal +
  ...
  const updateHeaderField = <K extends keyof EditableHeader>(
    key: K,
    value: EditableHeader[K],
  ) => {
  ...
  const buildHeaderPayload = (): ServerInvoiceUpdateHeaderPayload => {
    ...
    return {
      ...
      warehouse: hasChanged(source.warehouse, headerForm.warehouse)
        ...
        : null,
      branchCode: hasChanged(source.branchCode, headerForm.branchCode)
        ? headerForm.branchCode
        : null,
      cashier: hasChanged(source.cashier, headerForm.cashier)
        ? headerForm.cashier
        : null,
      issuedAt: hasChanged(
        toDateInputValue(source.issuedAt),
        headerForm.issuedAt,
      )
        ? headerForm.issuedAt
          ? new Date(headerForm.issuedAt).toISOString()
          : null
        : null,
      store: hasChanged(source.store, headerForm.store)
        ? headerForm.store
        : null,
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Factura #{invoice.header.document}
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    updateHeaderField("chargeStatus", e.target.value);
                  }}
              ...
              <MaterialInput
                ...
                onChange={(e) =>
                  updateHeaderField("clientCode", e.target.value)
                }
              />
              <MaterialInput
                ...
                onChange={(e) =>
                  updateHeaderField("clientName", e.target.value)
                }
              />
              <MaterialInput
                label="Bodega"
                ...
                onChange={(e) =>
                  updateHeaderField("warehouse", safeNumber(e.target.value))
                }
              />
              <MaterialInput
                label="Sucursal"
                ...
                onChange={(e) =>
                  updateHeaderField("branchCode", e.target.value)
                }
              />
              <MaterialInput
                label="Cajero"
              ...
              <MaterialInput
                ...
                onChange={(e) =>
                  updateHeaderField("promoterCode", e.target.value)
                }
              />
              <MaterialInput
                ...
                onChange={(e) =>
                  updateHeaderField("promoterName", e.target.value)
                }
              />
              <MaterialInput
              ...
              <MaterialInput
                ...
                onChange={(e) =>
                  updateHeaderField("coupon", safeNumber(e.target.value))
                }
              />
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <p className="font-mono text-foreground">
                  {computedTotals.grossTotal.toFixed(2)}
                </p>
              </div>
              <div>
                <span className="text-muted-foreground">Impuesto 1</span>
                <p className="font-mono text-foreground">
                  {computedTotals.tax1Total.toFixed(2)}
                </p>
              </div>
              <div>
                <span className="text-muted-foreground">Impuesto 2</span>
                <p className="font-mono text-foreground">
                  {computedTotals.tax2Total.toFixed(2)}
                </p>
              </div>
              <div>
                <span className="text-muted-foreground">Desc. Línea</span>
                <p className="font-mono text-foreground">
                  {computedTotals.lineDiscountTotal.toFixed(2)}
                </p>
              </div>
              <div>
                <span className="text-muted-foreground">Desc. General</span>
                <p className="font-mono text-foreground">
                  {computedTotals.generalDiscountTotal.toFixed(2)}
                </p>
              </div>
              <div>
                <span className="text-muted-foreground">Neto</span>
                <p className="font-mono text-primary">
                  <strong>{computedTotals.netTotal.toFixed(2)}</strong>
                </p>
              </div>
            </div>
          </div>
