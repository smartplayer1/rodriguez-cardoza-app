...
import { exportCollectionsReportToExcel, exportCollectionsReportToPdf } from './export';
...
export default function ExportButtons({
  ...
  const fetchAllRecords = async () => {
    ...
    const response = await fetch(`/api/billing/reports/collections?${params.toString()}`);
    if (!response.ok) {
      throw new Error('No se pudo obtener el reporte para exportar');
    }

    return (await response.json()) as CollectionsReportResponse;
  };
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'excel') {
        exportCollectionsReportToExcel(data.records.records, data.summary);
      } else {
        exportCollectionsReportToPdf(data.records.records, data.summary);
      }
    } catch (error) {
