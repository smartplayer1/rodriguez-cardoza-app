...
import { SalesInvoicesResponse } from '@/app/type/sales-report';
...
export default async function FacturasReportePage({
  searchParams,
}: {
  ...
  const includeVoided = toStringValue(resolvedSearchParams.includeVoided) === 'true';
  ...
  const filterFields = [
    ...
    ['cashier', toStringValue(resolvedSearchParams.cashier)],
    ['chargeStatus', toStringValue(resolvedSearchParams.chargeStatus)],
  ...
  let response: SalesInvoicesResponse;
  let fetchError: string | null = null;

  try {
    response = await getSalesInvoicesReport({
      dateFrom,
      dateTo,
      ...
      cashier: toStringValue(resolvedSearchParams.cashier) || undefined,
      chargeStatus: toStringValue(resolvedSearchParams.chargeStatus) || undefined,
      includeVoided,
      page,
      perPage,
      ...context,
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 p-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-foreground">Reporte de Facturas de Venta</h2>
                  <p className="text-muted-foreground">
                    Facturación emitida por sucursal, cliente, promotor y cajero.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-3">
              ...
              <StatCard label="Bruto C$" value={formatCurrency(summary.grossTotal)} icon={<Percent className="size-4" />} />
              <StatCard
                label="Descuento línea C$"
                value={formatCurrency(summary.lineDiscountTotal)}
                icon={<Percent className="size-4" />}
              />
              <StatCard
                label="Descuento general C$"
                value={formatCurrency(summary.generalDiscountTotal)}
                icon={<Percent className="size-4" />}
              />
              <StatCard label="Impuestos C$" value={formatCurrency(summary.tax1Total + summary.tax2Total)} icon={<Percent className="size-4" />} />
              <StatCard label="Neto C$" value={formatCurrency(summary.netTotal)} icon={<Percent className="size-4" />} />
              <StatCard label="Anuladas" value={summary.voidedCount} icon={<FileX2 className="size-4" />} />
            </div>
          </div>
        </header>
          ...
          <div className="mt-5">
            <form method="get" className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              <FilterInput
                label="Cajero"
                name="cashier"
                defaultValue={toStringValue(resolvedSearchParams.cashier)}
                placeholder="Cajero"
              />
              ...
              <label className="space-y-2">
                ...
                <select
                  name="chargeStatus"
                  defaultValue={toStringValue(resolvedSearchParams.chargeStatus)}
                  className="w-full rounded-2xl border border-border bg-background px-3 py-2 text-sm text-foreground outline-none transition-colors focus:border-primary"
                >
              ...
              <label className="flex items-center gap-2 self-end pb-2">
                ...
                />
                <span className="text-sm text-muted-foreground">Incluir facturas anuladas</span>
              </label>
            ...
            <table className="min-w-full divide-y divide-border/60 text-sm">
              <thead className="bg-muted/40">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-muted-foreground">Pago</th>
                  <th className="px-4 py-3 text-right font-medium text-muted-foreground">Bruto</th>
                  <th className="px-4 py-3 text-right font-medium text-muted-foreground">Descuento</th>
                  <th className="px-4 py-3 text-right font-medium text-muted-foreground">Impuesto</th>
                  <th className="px-4 py-3 text-right font-medium text-muted-foreground">Neto</th>
                ...
                {records.length > 0 ? (
                  records.map((record) => (
                    <tr
                      ...
                      className={`align-top transition-colors hover:bg-muted/15 ${record.isVoided ? 'opacity-60' : ''}`}
                    >
                      <td className="px-4 py-4 font-mono text-foreground">{record.document}</td>
                      <td className="px-4 py-4 text-xs text-muted-foreground">{formatDateTime(record.issuedAt)}</td>
                      ...
                      <td className="px-4 py-4 text-foreground">{record.cashier}</td>
                      <td className="px-4 py-4 text-center">
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-1 text-xs ${
                            record.chargeStatus === 'Contado'
                      ...
                      <td className="px-4 py-4 text-right font-mono text-foreground">{formatCurrency(record.grossTotal)}</td>
                      <td className="px-4 py-4 text-right font-mono text-foreground">{formatCurrency(record.discountTotal)}</td>
                      <td className="px-4 py-4 text-right font-mono text-foreground">{formatCurrency(record.taxTotal)}</td>
                      <td className="px-4 py-4 text-right font-mono text-foreground">{formatCurrency(record.netTotal)}</td>
                      <td className="px-4 py-4 text-center">
                        {record.isVoided ? (
                          <span className="inline-flex items-center rounded-full bg-rose-100 px-2 py-1 text-xs text-rose-700">
                            Anulada
                          </span>
                        ) : (
        ...
        {paging.totalPages > 1 ? (
          <nav aria-label="Paginación facturas" className="rounded-2xl border border-border/60 bg-surface px-4 py-3">
