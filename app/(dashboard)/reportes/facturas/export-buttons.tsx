...
import { exportSalesInvoicesToExcel, exportSalesInvoicesToPdf } from './export';
...
export default function ExportButtons({
  ...
  const fetchAllRecords = async () => {
    ...
    const response = await fetch(`/api/sales/reports/invoices?${params.toString()}`);
    if (!response.ok) {
      throw new Error('No se pudo obtener el reporte para exportar');
    }

    return (await response.json()) as SalesInvoicesResponse;
  };
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'excel') {
        exportSalesInvoicesToExcel(data.records.records, data.summary);
      } else {
        exportSalesInvoicesToPdf(data.records.records, data.summary);
      }
    } catch (error) {
