...
import { exportAgentsNearGoalsToExcel, exportAgentsNearGoalsToPdf } from './export';
...
export default function ExportButtons({
  ...
  const fetchAllRecords = async () => {
    ...
    const response = await fetch(`/api/reward/reports/agents-near-goals?${params.toString()}`);
    if (!response.ok) {
      throw new Error('No se pudo obtener el reporte para exportar');
    }

    return (await response.json()) as AgentsNearGoalsResponse;
  };
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'excel') {
        exportAgentsNearGoalsToExcel(data.records.records, data.summary);
      } else {
        exportAgentsNearGoalsToPdf(data.records.records, data.summary);
      }
    } catch (error) {
