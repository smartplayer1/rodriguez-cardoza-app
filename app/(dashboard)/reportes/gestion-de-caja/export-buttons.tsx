...
import { exportCashManagementReportToExcel, exportCashManagementReportToPdf } from './export';
...
export default function ExportButtons({
  ...
  const fetchAllRecords = async () => {
    ...
    const response = await fetch(`/api/billing/reports/cash-management?${params.toString()}`);
    if (!response.ok) {
      throw new Error('No se pudo obtener el reporte para exportar');
    }

    return (await response.json()) as CashManagementReportResponse;
  };
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'excel') {
        exportCashManagementReportToExcel(data.records.records, data.summary);
      } else {
        exportCashManagementReportToPdf(data.records.records, data.summary);
      }
    } catch (error) {
