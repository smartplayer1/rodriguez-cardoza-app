...
import { CouponMovementsReportResponse } from '@/app/type/reward-report';
import { exportCouponMovementsToExcel, exportCouponMovementsToPdf } from './export';
...
export default function ExportButtons({
  ...
  const fetchAllRecords = async () => {
    ...
    const response = await fetch(`/api/reward/reports/coupon-movements?${params.toString()}`);
    if (!response.ok) {
      throw new Error('No se pudo obtener el reporte para exportar');
    }

    return (await response.json()) as CouponMovementsReportResponse;
  };
  ...
  const handleExport = async (type: 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (type === 'excel') {
        exportCouponMovementsToExcel(data.records.records, data.summary);
      } else {
        exportCouponMovementsToPdf(data.records.records, data.summary);
      }
    } catch (error) {
