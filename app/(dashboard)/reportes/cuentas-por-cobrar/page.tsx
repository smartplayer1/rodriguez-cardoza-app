...
import { Banknote, ChevronDown, DollarSign, FileText, Receipt } from 'lucide-react';

import { getAccountsReceivableClientPromotersReport } from '@/app/services/billing/reports';
...
import { AccountsReceivableResponse } from '@/app/type/accounts-receivable-report';
...
export default async function CuentasPorCobrarPage({
  ...
  let response: AccountsReceivableResponse;
  let fetchError: string | null = null;

  try {
    response = await getAccountsReceivableClientPromotersReport({
      dateFrom,
      dateTo,
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 p-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-foreground">Cuentas por Cobrar - Cliente / Promotor</h2>
                  <p className="text-muted-foreground">
                    Facturas con saldo pendiente agrupadas por cliente y promotor.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
              ...
              <StatCard
                label="Total C$"
                value={formatCurrency(summary.invoiceTotalNio)}
              ...
              <StatCard
                label="Pendiente C$"
                value={formatCurrency(summary.pendingBalanceTotalNio)}
              ...
              <tbody className="divide-y divide-border/40">
                {records.length > 0 ? (
                  records.map((record, index) => (
                    <tr key={`${record.invoiceNumber}-${index}`} className="align-top transition-colors hover:bg-muted/15">
                      ...
                      <td className="px-4 py-4 font-mono text-foreground">{record.invoiceNumber}</td>
                      <td className="px-4 py-4 text-xs text-muted-foreground">{formatDate(record.invoiceDate)}</td>
                      <td className="px-4 py-4 text-xs text-muted-foreground">{formatDate(record.dueDate)}</td>
                      ...
                      <td className="px-4 py-4 text-foreground">
                        <div>{record.branchName ?? record.branchCode}</div>
                        {record.branchCanton && (
                          <div className="text-xs text-muted-foreground">{record.branchCanton}</div>
                        )}
                      </td>
                    </tr>
                  ))
        ...
        {paging.totalPages > 1 ? (
          <nav aria-label="Paginación cuentas por cobrar" className="rounded-2xl border border-border/60 bg-surface px-4 py-3">
