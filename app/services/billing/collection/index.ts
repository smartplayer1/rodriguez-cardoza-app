import { CollectionCreatePayload, CollectionListResponse, CollectionRecord } from '@/app/type/collection';
...
const buildQueryString = (filters?: Omit<CollectionFilters, 'baseUrl'>) => {
  ...
  if (filters.invoiceId?.trim()) params.set('invoiceId', filters.invoiceId.trim());
  if (filters.invoiceDocument?.trim()) params.set('invoiceDocument', filters.invoiceDocument.trim());
  if (filters.cashRegisterId?.trim()) params.set('cashRegisterId', filters.cashRegisterId.trim());
  if (filters.cashManagementId?.trim()) params.set('cashManagementId', filters.cashManagementId.trim());
  if (filters.currency?.trim()) params.set('currency', filters.currency.trim());
  if (filters.applicationStatus?.trim()) params.set('applicationStatus', filters.applicationStatus.trim());
  if (filters.status?.trim()) params.set('status', filters.status.trim());
  if (filters.collectionDate?.trim()) params.set('collectionDate', filters.collectionDate.trim());
