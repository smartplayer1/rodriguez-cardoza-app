...
const readErrorMessage = (body: unknown) => {
  if (!body || typeof body !== 'object') {
    return 'Error al cerrar el ledger de recompensas del cliente';
  }

  const errorBody = body as { detail?: string; message?: string; error?: string };
  return errorBody.detail || errorBody.message || errorBody.error || 'Error al cerrar el ledger de recompensas del cliente';
};
...
export async function POST(
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/v1/reward/client/${clientCode}/reward-ledger/close`,
    {
      method: 'POST',
      headers: {
