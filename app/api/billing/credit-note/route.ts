...
const readErrorMessage = (body: unknown) => {
  if (!body || typeof body !== 'object') {
    return 'Error al procesar nota de credito';
  }

  const errorBody = body as { detail?: string; message?: string; error?: string };
  return errorBody.detail || errorBody.message || errorBody.error || 'Error al procesar nota de credito';
};
...
export async function GET(req: Request) {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/v1/billing/credit-note${queryString ? `?${queryString}` : ''}`,
    {
      method: 'GET',
      headers: {
