...
export async function GET(req: Request) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/v1/reports/accounts-receivable/client-promoters${queryString ? `?${queryString}` : ""}`,
    {
      method: "GET",
      headers: {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      { message: errorData?.detail || "Error al obtener el reporte de cuentas por cobrar" },
      { status: res.status },
    );
  }
