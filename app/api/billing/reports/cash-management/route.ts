...
export async function GET(req: Request) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/v1/reports/billing/cash-management${queryString ? `?${queryString}` : ""}`,
    {
      method: "GET",
      headers: {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      { message: errorData?.detail || "Error al obtener el reporte de gestión de caja" },
      { status: res.status },
    );
  }
